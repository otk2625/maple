...
public class MiniMushroom extends Enemy {
	
	
	public MiniMushroom() {
	}

	public MiniMushroom(String string, int x, int y, int hp, String name) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				while (true) {
					...
					if (moveState == 1) {
						setIcon(new ImageIcon("image/작은주황버섯.gif"));
						x++;
						...
						setIcon(new ImageIcon("image/작은주황버섯왼쪽.gif"));
						x--;
						if (x <= 05) {
							moveState = 1;
						}
