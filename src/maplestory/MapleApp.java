...
public class MapleApp extends JFrame implements Initable {
	...
	public void init() {
		...
		mushroom = new Mushroom("image/ÁÖÈ²¹ö¼¸.gif", 555, 380, 20, "ÁÖÈ²¹ö¼¸");
		stone = new Stone("image/Stone.gif", 100, 150, 20, "½ºÅæ°ñ·½");
		barlog = new Barlog("image/¹ß·Ï¿À¸¥ÂÊ.gif", 200, -20, 20, "ÁÖ´Ï¾î¹ß·Ï");
		miniMushroom = new MiniMushroom("image/ÀÛÀºÁÖÈ²¹ö¼¸.gif", 200, 400, 20, "¹Ì´ÏÁÖÈ²¹ö¼¸");
		...
		balCol.start();
		Thread miniMushroomCol = new Thread(new col(miniMushroom));
		miniMushroomCol.start();
				...
				try {
					...
					if (isBossStart == true) {
						...
						if (isBoss == true) {
							...
							mushroom.x = 99999;
							stone.x = 99999;
							miniMushroom.x = 99999;
							barlog.x = 99999;
							c.remove(miniMushroom);
							c.remove(stone);
							c.remove(mushroom);
							c.remove(barlog);
						...
						if (player.isAttack == true) {
							...
							if (enemy.hp <= 0) {
								...
								if (enemy == boss && enemy.hp < 380) {
									enemy.setIcon(new ImageIcon("image/ÀÚÄñ°³ÇÇ.png"));
								}
							}
